...
from CorrectorPlayEnv_v5 import CorrectorPlayEnv  # 你原来的 simulator
# -----------------------------
...
class C1A1ObservationWrapper(gym.ObservationWrapper):
    """
    Restrict observations to only [C1, A1] for the low-level C1A1 policy.
    """

    def __init__(self, env: gym.Env):
        super().__init__(env)
        self.keep_idx = np.array([0, 1], dtype=np.int64)
...
def extract_c1a1_obs(obs: np.ndarray) -> np.ndarray:
...
def make_low_env(mode: str, seed: int, action_idx: List[int],
    ...
    def _init():
        ...
        env.reset(seed=seed)
        if mode == "C1A1":
            env = C1A1ObservationWrapper(env)
...
class AlternatingHighEnv(gym.Env):
    ...
    def _run_c1a1_until_gate(self, obs):
        ...
        while (not self._c1a1_gate_ok(obs)) and (steps < self.n_c1a1_max):
            ...
            obs_batch = extract_c1a1_obs(obs)[None, :]  # (1,2)
...
def evaluate_low_level_model(
    ...
    obs_names = ["C1", "A1"] if mode == "C1A1" else OBS_NAMES
    ...
    for t in range(max_steps):
        ...
        o_after = obs2[0].copy()
        if d and isinstance(info, dict) and ("terminal_observation" in info):
            o_after = np.array(info["terminal_observation"], dtype=np.float32).copy()
...
if __name__ == "__main__":
    ...
    env_kwargs = dict(
        ...
        user_beta={},
        user_sigma={'C1': 10, 'A1': 10, 'A2': 100, 'B2': 50, 'C3': 200, 'A3': 100, 'S3': 100},
    ...
    #
    # c1a1_path = '/Users/haozhi/Codes/Mic_Hackathon_2025/RL_STEM-main/two_steps_model_v1/runs_hier_rppo/c1a1_rppo_03-01-26-1048.zip'
    ...
    save_path = train_high_order_with_c1a1_guard(
        ...
        n_high=2,  # 每次 high-order 决策执行几次同样的 high 动作
        c1_gate=1.2,
        a1_gate=2.5,
    ...
    evaluate_alternating_high_model(
        out_csv_dir=r'/Users/haozhi/Codes/Mic_Hackathon_2025/RL_STEM-main/result/SR',
        high_model_path=save_path,
        c1a1_model_path=c1a1_path,
        env_kwargs=env_kwargs,
        n_episodes=3,
        seed=0,
        ...
        deterministic_c1a1=True,    # conditioning 推荐 True
    )
