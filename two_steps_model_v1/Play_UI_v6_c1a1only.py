...
class PlayUI:
    ...
    def __init__(self, root, env: CorrectorPlayEnv):
        ...
        for c in range(2):  # Only 2 columns now (A1, C1)
            self.ctrl_frame.columnconfigure(c, weight=1)

        # MAIN pct controls (COMMENTED OUT - high-order)
        ...
        self.a1_ctrl.grid(row=0, column=0, sticky="ew", padx=(0, 8))
        ...
        self.c1_ctrl.grid(row=0, column=1, sticky="ew", padx=(0, 8))
        ...
        # C3 pct controls (COMMENTED OUT - high-order)
    ...
    # COMMENTED OUT - high-order action (MAIN)
    ...
    # COMMENTED OUT - high-order action (C3)
    ...
    def _format_values_line(self) -> str:
        # Show only low-order values (C1, A1)
        low_order_keys = ["C1", "A1"]
        parts = [f"{k}={self.env.params[k]:.6g}" for k in low_order_keys if k in self.env.params]
