...
class CorrectorPlayEnv(gym.Env):
    ...
    # C1_STEPS = [1, 5, 10, 15, 20, 50, 75, 100]
    ...
    def _build_action_table(self):
        ...
        # Change C3 from step to pct button
        ...
        # Change C1 from step to pct button
    ...
    def _sample_init_params(self):
        ...
        for k in self.KEYS:
            ...
            if k not in ["C1", "C3"]:
                ...
                init_params[k] = v * random.choice([1, -1])
        return init_params
    ...
    def step(self, action):
        ...
        if act["type"] == "pct_button":
            if not failed:
                ...
                #     delta['A1'] = -1.323260*self.params['A1'] + float(self.error_rng.normal(0.0, base_sig))
                if target in ['C1', 'C3']:
                    target_change = -np.sign(val) * (base_mag + float(self.error_rng.normal(0.0, base_sig))) * (pct / 100.0)
                    delta[target] = target_change
                else:
                    target_change = -(base_mag + float(self.error_rng.normal(0.0, base_sig))) * (pct / 100.0)
                    delta[target] = target_change
        ...
        # Reward: bounded tanh reward (keeps rewards in reasonable range)
        # Goal is to minimize KEYS in the sub-action space to zero, changed on 12/29/2025

        max_dev = max(abs(self.params[k]) for k in self.explode_keys)
        total_dev = sum(abs(self.params[k]) for k in self.explode_keys)

        # Tanh-based reward: bounded between -1 and 0 per step
        # - When params are large, reward ≈ -1
        # - When params are near 0, reward ≈ 0
        # reward = -np.tanh(total_dev / 5000.0)
        reward = 0.0  # disable default reward

        # Alternative: combined max + sum (unbounded, can cause training issues)
        ...
        # Large penalty for explosion - agent must learn this is BAD
        if deviation_exploded:
            reward = -1000.0  # Heavy penalty to discourage explosive actions
