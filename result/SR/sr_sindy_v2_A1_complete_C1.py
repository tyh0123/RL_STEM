...
# Config
# ============================================================
CSV_PATH = "result/SR/A1_actions.csv"
FIG_DIR = "sindy_validation_plots_A1_actions_C1"
...
# ============================================================
def print_clean_sindy_equation(model, target_name="C1_afteraction", tol=1e-10):
...
# ------------------------------------------------------------
y_all = df["C1_afteraction"].to_numpy(float)
p_low, p_high = 3, 97
...
y = df["C1_afteraction"].to_numpy(float).reshape(-1, 1)
y_true = y.ravel()
...
print_clean_sindy_equation(model, "C1_afteraction", TOL)
...
plt.xlabel("Actual (C1 After Action)")
plt.ylabel("Predicted (C1 After Action)")
...
# Plot 2: 95% range
# ============================================================
x_lo, x_hi = np.percentile(y_true, [0, 95])
y_lo, y_hi = np.percentile(y_pred, [0, 95])
...
plt.xlabel("Actual (C1 After Action)")
plt.ylabel("Predicted (C1 After Action)")
plt.title("SINDy: Actual vs Predicted (95%)")
...
plt.tight_layout()
plt.savefig(
    os.path.join(FIG_DIR, "actual_vs_predicted_95pct.png"),
    dpi=300
)
plt.close()
